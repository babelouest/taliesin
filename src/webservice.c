...
int callback_taliesin_server_configuration (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  set_response_json_body_and_clean(response, 200, json_pack("{ss*ss*ss*ss*sisisisssososs*}", 
                        ...
                        "default_stream_format", ((struct config_elements *)user_data)->stream_format,
                        "default_stream_channels", ((struct config_elements *)user_data)->stream_channels,
            ...
            if (j_valid != NULL && json_array_size(j_valid) == 0) {
              if (u_map_get(request->map_url, "webradio") != NULL) {
                j_stream_info = add_webradio_from_path(config,
                                                       ...
                                                       decode_path,
                                                       (0==o_strcmp(TALIESIN_SCOPE_ALL, u_map_get(request->map_url, "scope")))?NULL:get_username(request, response, config),
                                                       format,
                                                       channels,
                                                       sample_rate,
                                                       bit_rate,
                ...
                j_stream_info = add_jukebox_from_path(config,
                                                      u_map_get(request->map_url, "streamUrl"),
                                                      json_object_get(j_data_source, "data_source"),
                                                      decode_path,
                                                      (0==o_strcmp(TALIESIN_SCOPE_ALL, u_map_get(request->map_url, "scope")))?NULL:get_username(request, response, config),
          ...
          if (u_map_get(request->map_url, "webradio") != NULL || u_map_get(request->map_url, "jukebox") != NULL) {
            ...
            if (j_valid != NULL && json_array_size(j_valid) == 0) {
              ...
              if (u_map_get(request->map_url, "webradio") != NULL) {
                ...
                j_fake_jukebox = json_pack("{sssssisO}", "description", u_map_get(request->map_url, "category"), "name", u_map_get(request->map_url, "category"), "tpl_id", 0, "media", json_object_get(j_result, "media"));
                j_stream_info = add_jukebox_from_playlist(config,
                                                          u_map_get(request->map_url, "streamUrl"),
                                                          j_fake_jukebox,
                                                          (0==o_strcmp(TALIESIN_SCOPE_ALL, u_map_get(request->map_url, "scope")))?NULL:get_username(request, response, config),
                                                          format,
                                                          channels,
                                                          sample_rate, bit_rate, u_map_get(request->map_url, "name"));
                if (check_result_value(j_stream_info, T_OK)) {
                  ulfius_set_json_body_response(response, 200, json_object_get(j_stream_info, "stream"));
                } else {
                  y_log_message(Y_LOG_LEVEL_ERROR, "callback_taliesin_media_get_path - Error creating jukebox");
            ...
            if (client_data_webradio->audio_stream->nb_client_connected >= TALIESIN_STREAM_CLIENT_MAX) {
              ...
              if (client_data_webradio->audio_stream->client_list != NULL) {
                ...
                if (ulfius_set_stream_response(response, 200, u_webradio_stream, u_webradio_stream_free, U_STREAM_SIZE_UNKOWN, client_data_webradio->audio_stream->stream_bitrate / 8, client_data_webradio) == U_OK) {
                  ...
                  ulfius_add_header_to_response(response, "Content-Type", config_get_content_type_from_path(config, "stream.mp3"));
              ...
              if (client_data_jukebox != NULL) {
                ...
                if (init_client_data_jukebox(client_data_jukebox) == T_OK) {
                  ...
                  if (client_data_jukebox->audio_buffer != NULL && jukebox_audio_buffer_init(client_data_jukebox->audio_buffer) == T_OK) {
                    ...
                    j_valid = is_stream_libav_parameters_valid(config, 0, 0, client_data_jukebox->stream_format, client_data_jukebox->stream_channels, client_data_jukebox->stream_sample_rate, client_data_jukebox->stream_bitrate);
                    if (j_valid != NULL && json_array_size(j_valid) == 0) {
                      if (0 == o_strcmp(json_string_value(json_object_get(json_object_get(j_media, "media"), "type")), "audio")) {
                        ...
                        ulfius_set_string_body_response(response, 400, "Invalid file format");
                        response->status = 400;
                      }
                    } else if (j_valid != NULL && json_array_size(j_valid) > 0) {
                      ulfius_set_json_body_response(response, 400, j_valid);
                    } else {
                      y_log_message(Y_LOG_LEVEL_ERROR, "callback_taliesin_stream_media - Error is_stream_libav_parameters_valid");
                      response->status = 500;
                    }
                    json_decref(j_valid);
...
void callback_websocket_stream_manager (const struct _u_request * request, struct _websocket_manager * websocket_manager, void * websocket_user_data) {
  ...
  if (ws_stream->webradio != NULL) {
    ...
    while (ulfius_websocket_wait_close(websocket_manager, 50) == U_WEBSOCKET_STATUS_OPEN && ws_stream->webradio->audio_stream->status != TALIESIN_STREAM_STATUS_STOPPED) {
      
      time(&now);
      if (((ws_stream->is_authenticated && ws_stream->expiration && ws_stream->expiration > now) || !ws_stream->config->use_oidc_authentication)) {
    ...
    // Loop until websocket is closed by the server or the client
    while (ws_stream->jukebox->message_type != TALIESIN_PLAYLIST_MESSAGE_TYPE_CLOSING && ulfius_websocket_wait_close(websocket_manager, 50) == U_WEBSOCKET_STATUS_OPEN) {
...
int callback_taliesin_config_type_get (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  if (0 == o_strcmp(TALIESIN_CONFIG_EXTERNAL_PLAYER, u_map_get_case(request->map_url, "type"))) {
