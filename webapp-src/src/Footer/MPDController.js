			...
			.then((status) => {
				this.setState({jukeboxRepeat: status.repeat, jukeboxRandom: status.random, volume: status.volume, jukeboxIndex: status.song_pos, play: (status.state==="play")}, () => {
					...
					.then((result) => {
						...
						if (result.uri && (result.uri.startsWith(StateStore.getState().taliesinApiUrl + "/stream") || result.uri.startsWith(StateStore.getState().serverConfig.icecast_remote_address))) {
							var streamName;
              if (result.uri.startsWith(StateStore.getState().taliesinApiUrl + "/stream")) {
                ...
                streamName = result.uri.substring((StateStore.getState().serverConfig.icecast_remote_address).length);
              }
