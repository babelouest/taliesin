...
class Element extends Component {
  constructor(props) {
    super(props);
		this.state = {element: props.element, dataSource: props.dataSource, browse: props.browse, path: props.path, container: props.container, thumb: false, thumbLoaded: false, visible: false};
	...
	componentWillReceiveProps(nextProps) {
		this.setState({element: nextProps.element, dataSource: nextProps.dataSource, browse: nextProps.browse, path: nextProps.path});
	}
  ...
  handleOpenFile(name) {
	...
	getThumbnail() {
		if (!this.state.thumb && (this.state.element.type === "audio" || this.state.element.type === "folder" || this.state.element.type === "image" || this.state.element.type === "artist" || this.state.element.type === "album" || this.state.element.type === "year" || this.state.element.type === "genre")) {
			StateStore.getState().APIManager.taliesinApiRequest("GET", "/data_source/" + this.state.dataSource + "/browse/" + this.state.browse + "/" + this.state.path + "/" + this.state.element.name + "?cover&thumbnail&base64")
			.then((result) => {
	...
	render() {
		...
		return (
			<Col md={2} xs={6} className="panel panel-default">
        <VisibilitySensor
          scrollCheck
          scrollThrottle={100}
          intervalDelay={8000}
